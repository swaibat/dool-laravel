...
class PayPalHelper
{
    ...
    {
        $this->_http->addHeader("PayPal-Partner-Attribution-Id: " . getenv('SBN_CODE'));
    }
    ...
    private function _createApiUrl($resource)
    {
        if ($resource == 'oauth2/token')
            ...
            return $this->_apiUrl . "/" . getenv('PAYPAL_REST_VERSION') . "/" . $resource;
    }
    ...
    private function _getToken()
    {
        ...
        $this->_http->setAuthentication(getenv('PAYPAL_CREDENTIALS.' . getenv("PAYPAL_ENVIRONMENT") . '.client_id') . ":" . getenv('PAYPAL_CREDENTIALS.' . getenv("PAYPAL_ENVIRONMENT") . '.client_secret'));
